...
export type VariableToken = { id: string; name?: string; description?: string; collectionId: string; type: string; cssName?: string; scopes?: string[]; valuesByMode: Record<string, { kind: string; value?: any; tokenId?: string }> };
...
export function variableExpression(token: VariableToken, property: string, alpha = 1) {
  ...
  if (token.type === "number" && !UNITLESS_PROPERTIES.has(property)) return `calc(${expression} * 1px)`;
  if (token.type === "color" && alpha !== 1) return `color-mix(in srgb, ${expression} ${Math.round(alpha * 10000) / 100}%, transparent)`;
  return expression;
}
export function literalForProperty(value: any, token: VariableToken, property: string, alpha = 1) {
  if (token.type === "number") return UNITLESS_PROPERTIES.has(property) ? value : `${value}px`;
  return token.type === "color" && alpha !== 1 ? `color-mix(in srgb, ${value} ${alpha * 100}%, transparent)` : value;
}
  ...
  for (const token of library.tokens) {
    ...
    for (const mode of collection.modes) {
      ...
      if (value.kind === "alias") {
        ...
        if (token.type === "number" && (typeof value.value !== "number" || !Number.isFinite(value.value))) throw variableError("VARIABLE_VALUE_INVALID", "Enter a finite number.");
...
export function detachElementVariable(element: any, library: VariableLibrary, property: string, values: Record<string, any>) {
  const binding = element.theme?.bindings?.find(item => item.target === "style" && item.property === property);
  ...
  return { ...element, theme: { ...element.theme, bindings: element.theme.bindings.filter(item => item !== binding) }, styles: { ...element.styles, [property]: literalForProperty(values[token.id], token, property, binding.alpha) } };
}
