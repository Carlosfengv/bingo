...
function VariableValueCell({ token, mode, disabled, onCommit, tokens }) {
  ...
  const saveValue = text => {
    const next = token.type === "number" ? Number(text) : token.type === "boolean" ? text === "true" : text;
    if (token.type === "color" && !CSS.supports("color", text)) { setError(t("variables.invalidColor")); return; }
    if (token.type === "number" && (!text.trim() || !Number.isFinite(next))) { setError(t("variables.number")); return; }
  ...
  return <div className="flex min-w-0 flex-col gap-1">
    <div className="flex min-w-0 items-center gap-1">
      {aliasOpen ? <select aria-label={`${t("variables.alias")} · ${label}`} className={`${input} w-full`} value={value?.tokenId || ""} disabled={disabled} onChange={event => { if (event.target.value) onCommit({ kind: "alias", tokenId: event.target.value }); }}>
        ...
        {token.type === "color" && <span className="size-4 shrink-0 rounded-sm border border-ed-border" style={{ backgroundColor: value?.value }} />}
...
export function VariableManager() {
  ...
  return <Dialog open={variables.managerOpen} onOpenChange={open => { if (!open) variables.closeManager(); }}>
    <DialogContent style={{ width: "min(1100px, 94vw)", height: "min(720px, 88vh)", maxWidth: "94vw" }} className="flex h-[min(720px,88vh)] w-[min(1100px,94vw)] max-w-none flex-col gap-0 overflow-hidden border-ed-border bg-ed-background p-0 text-ed-foreground sm:max-w-none" data-text-edit-safe="true" onKeyDown={event => {
      ...
      {variables.cssSource && <p className="border-b border-ed-border px-5 py-2 text-xs text-ed-muted-foreground">{t("variables.cssSource")}</p>}
      <div className="flex min-h-0 flex-1">
        <aside className="flex w-48 shrink-0 flex-col gap-1 overflow-y-auto border-r border-ed-border p-3">
          ...
          {library.collections.map(item => <button type="button" key={item.id} className={`${button} justify-start truncate ${collection?.id === item.id ? "bg-ed-muted font-medium" : ""}`} onClick={() => { setCollectionId(item.id); setGroup(""); }}>{item.name || item.id}</button>)}
        ...
        <main className="flex min-w-0 flex-1 flex-col">
          {!collection ? <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center"><p className="max-w-sm text-sm leading-6 text-ed-muted-foreground">{t("variables.empty")}</p><button type="button" className={`${button} border border-ed-border`} disabled={disabled} onClick={addCollection}><Plus className="size-3.5" />{t("variables.newCollection")}</button></div> : <>
            <div className="flex flex-wrap items-center gap-2 border-b border-ed-border p-3">
              <EditableText label={t("variables.collection")} className="w-36 border-transparent font-medium" value={collection.name || collection.id} disabled={disabled} onCommit={name => { if (name.trim()) edit(next => { next.collections.find(item => item.id === collection.id).name = name.trim(); return next; }); }} />
              ...
              <button type="button" className={button} disabled={disabled} onClick={() => addMode(collection.defaultModeId)}><Plus className="size-3.5" />{t("variables.newMode")}</button>
            </div>
            <div className="min-h-0 flex-1 overflow-auto">
              <table className="w-full border-collapse text-xs"><thead className="sticky top-0 z-10 bg-ed-background"><tr>
                ...
                {collection.modes.map((mode, index) => <th key={mode.id} className="min-w-48 border-b border-r border-ed-border p-2 text-left font-medium">
                  <div className="flex items-center gap-1"><EditableText label={`${t("variables.modes")} · ${mode.name || mode.id}`} className="w-full border-transparent" value={mode.name || mode.label || mode.id} disabled={disabled} onCommit={name => { if (name.trim()) edit(next => { next.collections.find(item => item.id === collection.id).modes.find(item => item.id === mode.id).name = name.trim(); return next; }); }} />{index === 0 && <span className="shrink-0 whitespace-nowrap text-[10px] text-ed-muted-foreground">{t("variables.default")}</span>}</div>
                  ...
                  ].map(([Icon, key, action, unavailable]: any) => <button type="button" key={key} className={`${button} !h-6 !px-1.5`} aria-label={`${t(`variables.${key}`)} · ${mode.name || mode.id}`} title={t(`variables.${key}`)} disabled={disabled || unavailable} onClick={action}><Icon className="size-3" /></button>)}</div>
                </th>)}
              </tr></thead><tbody>
                {visible.map(token => <tr key={token.id} className="group hover:bg-ed-muted/30">
                  <td className="border-b border-r border-ed-border/60 px-3 py-2"><div className="flex items-center gap-1"><EditableText label={`${t("variables.name")} · ${token.name || token.id}`} value={token.name || token.id} className="w-full border-transparent bg-transparent" disabled={disabled} onCommit={name => { if (name.trim()) updateToken(token.id, { name: name.trim() }); }} /><button type="button" className={`${button} !px-1`} aria-label={`${t("variables.edit")} · ${token.name || token.id}`} onClick={() => setDetailId(detailId === token.id ? null : token.id)}>···</button></div></td>
                  {collection.modes.map(mode => <td key={mode.id} className="border-b border-r border-ed-border/60 px-3 py-2"><VariableValueCell token={token} mode={mode} disabled={disabled} tokens={library.tokens} onCommit={value => edit(next => { next.tokens.find(item => item.id === token.id).valuesByMode[mode.id] = value; return next; })} /></td>)}
                </tr>)}
              </tbody></table>
              {!visible.length && <p className="p-6 text-xs text-ed-muted-foreground">{t("variables.noMatches")}</p>}
            </div>
            <div className="flex shrink-0 gap-2 border-t border-ed-border p-3"><button type="button" className={button} disabled={disabled} onClick={() => addVariable("color")}><Plus className="size-3.5" />{t("variables.color")}</button><button type="button" className={button} disabled={disabled} onClick={() => addVariable("number")}><Plus className="size-3.5" />{t("variables.number")}</button></div>
          </>}
        </main>
        {detail && <aside className="flex w-56 shrink-0 flex-col gap-3 overflow-auto border-l border-ed-border p-4">
          ...
          <label className="flex flex-col gap-1 text-xs">{t("variables.description")}<EditableText label={t("variables.description")} value={detail.description || ""} disabled={disabled} onCommit={description => updateToken(detail.id, { description })} /></label>
          <fieldset className="flex flex-col gap-2 text-xs" disabled={disabled}><legend className="mb-2">{t("variables.scope")}</legend>
