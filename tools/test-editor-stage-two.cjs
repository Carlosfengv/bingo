...
app.whenReady().then(async () => {
  let editor;
  const output = path.join(repo, 'output/playwright/phase-two/regression');
  try {
    ...
    check('A numeric variable change refreshes the affected root geometry', report.layoutMode['元素盒测量'] >= 2500);
